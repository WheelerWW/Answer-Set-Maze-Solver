Hello.
This is my submission for my maze solver program using Answer Sets (to solve the maze) and TKinter Python (to get user input and generate the GUI).
...
It is rather irritating.  However, it is definitely a problem in the ASP, not the Python.  I have tried hard-coding length 3 mazes into the ASP, and the Answer Sets still make no sense.
Be prepared to see mazes that move through walls, somehow have 8 or more moves, move backwards, or move from one side of the maze to the other Pacman-style when doing sufficiently small mazes.  
(I honestly don't even understand that last one).  I have constraints to prevent all of these possibilities.  I do not know why they are ineffective at such small maze lengths.

I have also added example photos into this folder as an example to show what I mean.
In the "Not Working" photo, the maze should go from cell (4,4) to cell (4,6), a simple 2-movements maze solution.  Instead, it terminates at cell (6,6), and breaks through walls in order to do so.
Also in that photo, you can see that my ASP on the left is out of order, with Step(0) appearing last.  